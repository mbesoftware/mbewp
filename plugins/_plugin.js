...
Mbe.addCommand({pattern: 'install ?(.*)', fromMe: true, desc: Lang.INSTALL_DESC, warn: Lang.WARN}, (async (message, match) => {
    ...
    if (response.statusCode == 200) {
        ...
        try {
            ...
            fs.unlinkSync('/root/mbewp/plugins/' + plugin_name + '.js')
        ...
        if (find.includes(plugin_name)) {
        ...
        else if (response.body.includes('formation') && !match[1].includes('mbesoftware')) {
            ...
            if (!match[1].includes('mbesoftware') && DEG.level > 99) {
                await message.client.sendMessage(message.jid,LANG.limit + DEG.level + '_', MessageType.text)
                fs.unlinkSync('/root/mbewp/plugins/' + plugin_name + '.js')
            }
            else if (!match[1].includes('mbesoftware') && DEG.level < 100) {
...
Mbe.addCommand({pattern: 'plugin$', fromMe: true, desc: Lang.PLUGIN_DESC}, (async (message, match) => {
...
Mbe.addCommand({pattern: 'remove(?: |$)(.*)', fromMe: true, desc: Lang.REMOVE_DESC}, (async (message, match) => {
